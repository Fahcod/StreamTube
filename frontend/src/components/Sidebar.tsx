import { BiDownload, BiHistory, BiHome, BiPlayCircle, BiPlusCircle, BiSolidVideos } from "react-icons/bi";
...
const Sidebar = () => {
  ...
  return (
    <div className="md:block bg-white hidden px-6 w-[236px] overflow-y-scroll fixed h-screen pb-24 [&::-moz-scrollbar]:w-0 [&::-webkit-scrollbar]:w-0">
    {/* the links */}
    <div className="w-full flex flex-col gap-6 pt-4">
    
    <Link to="/home">
    <div className="flex text-[#202020] items-center gap-3 cursor-pointer">
    <BiHome className="w-6.5 h-6.5"/>
    <p className="text-[17px] font-[Arial] pt-0.5">Home</p>
    </div>
    </Link>

    <div onClick={()=>pickReel()} className="flex text-[#202020] items-center gap-3 cursor-pointer">
    ...
    <hr className="mt-0.5 border-none outline-none h-[1px] bg-gray-200"/>
    ...
    <Link to="/studio">
     <div className="flex items-center gap-3 cursor-pointer">
    <BiPlayCircle className="w-6.5 h-6.5"/>
    ...
    {all_users.map((item,index)=>{
       if(userData?.following.includes(item._id)){
        return (
        ...
        {/* the user info */}
        <div className="">
        <h1 className="font-[roboto-light] text-sm text-[#101010]">{item.username}</h1>
        <p className="text-[#454545] text-[13px] font-[roboto-light] leading-none">{item.followers.length} followers</p>
        </div>
        </div>
        )
       }
    ...
    <div className="cursor-pointer flex items-center gap-2 pt-5">
    <FaGear className="text-[#101010] w-5 h-5"/>
