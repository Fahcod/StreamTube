...
const SavedVideo = (props:VideoPostModel) => {
  return (
    ...
    <h1 className="font-[roboto-medium] hidden md:block">{props.title.slice(0,55)}</h1>
     <h1 className="font-[roboto-medium] leading-none md:hidden text-sm">{props.title.slice(0,36)}{props.title.length>36?'...':''}</h1>
    <BiDotsVerticalRounded className="flex-shrink-0 w-6 h-6 cursor-pointer"/>
    </div>
